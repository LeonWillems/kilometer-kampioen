...
An implementation of BFS to find the shortest path between intercity stations.

However, the shortest path is in terms of nodes (stations) visited, not the
actual distance travelled. The new implementation is Dijkstra's Algorithm,
which is guaranteed to find the smallest distance path between two stations.
...
if __name__ == "__main__":
    # Example usage. Note that the answer is not the shortest path in terms
    # of distance, but in terms of nodes travelled
    distances = {
        'a': {'b': 1, 'f': 2},
        'b': {'a': 1, 'c': 1},
        'c': {'b': 1, 'd': 1},
    ...
    bfs = BFS(adjacency_list=distances)
    path, distance = bfs.search(start, goal)
